    ...
    case lists:member(<<"ao-types">>, Subpaths) of
        true ->
            case hb_store:read(
                ...
                not_found ->
                    % Some messages (or partial cache entries) do not carry ao-types.
                    % Treat this as "no type hints" rather than crashing callers.
                    ?event({ao_types_not_found, {path, Path}}),
                    {ok, #{}, #{}};
                {error, Err} ->
                    ?event({ao_types_read_error, {path, Path}, {err, Err}}),
                    {ok, #{}, #{}}
            end;
        false ->
