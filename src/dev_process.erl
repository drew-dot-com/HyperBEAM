...
init(Base, Req, Opts) ->
    _ = file:write_file(
        "/tmp/dev_process_init.log",
        io_lib:format("~p init_start~n", [erlang:monotonic_time(millisecond)]),
        [append]
    ),
    ...
    {ok, Initialized} =
        dev_process_lib:run_as(
            ...
            Base,
            #{ <<"path">> => <<"init">> },
            Opts#{ force_lua_init => true, disable_send_init => true }
        ),
    _ = file:write_file(
        "/tmp/dev_process_init.log",
        io_lib:format("~p init_run_as_done~n", [erlang:monotonic_time(millisecond)]),
        [append]
    ),
    {
        ok,
...
compute(Base, Req, Opts) ->
    ...
    case TargetSlot of
        ...
        RawSlot ->
            ...
            _ = file:write_file(
                "/tmp/dev_process_compute_path.log",
                io_lib:format("~p cache_read_start slot=~p~n", [erlang:monotonic_time(millisecond), Slot]),
                [append]
            ),
            CacheRes = dev_process_cache:read(ProcID, Slot, Opts),
            _ = file:write_file(
                "/tmp/dev_process_compute_path.log",
                io_lib:format("~p cache_read_done slot=~p~n", [erlang:monotonic_time(millisecond), Slot]),
                [append]
            ),
            case CacheRes of
                {ok, Result} ->
...
compute_to_slot(ProcID, Base, Req, TargetSlot, Opts) ->
    ...
    _ = file:write_file(
        "/tmp/dev_process_compute_path.log",
        io_lib:format("~p compute_to_slot current=~p target=~p~n", [erlang:monotonic_time(millisecond), CurrentSlot, TargetSlot]),
        [append]
    ),
    ?event(compute_short,
        {starting_compute,
    ...
    case CurrentSlot of
        ...
        CurrentSlot ->
            ...
            _ = file:write_file(
                "/tmp/dev_process_compute_path.log",
                io_lib:format("~p next_start slot=~p~n", [erlang:monotonic_time(millisecond), NextSlot]),
                [append]
            ),
            case next(Base, Req, Opts) of
                {error, Res} ->
                    ...
                    _ = file:write_file(
                        "/tmp/dev_process_compute_path.log",
                        io_lib:format("~p next_error slot=~p~n", [erlang:monotonic_time(millisecond), NextSlot]),
                        [append]
                    ),
                    ?event(compute,
                ...
                {ok, #{ <<"body">> := SlotMsg, <<"state">> := State }} ->
                    ...
                    _ = file:write_file(
                        "/tmp/dev_process_compute_path.log",
                        io_lib:format("~p next_ok slot=~p~n", [erlang:monotonic_time(millisecond), NextSlot]),
                        [append]
                    ),
...
compute_slot(ProcID, State, RawInputMsg, ReqMsg, Opts) ->
    ...
    _ = file:write_file(
        "/tmp/dev_process_compute_path.log",
        io_lib:format("~p compute_slot_start slot=~p~n", [erlang:monotonic_time(millisecond), NextSlot]),
        [append]
    ),
    ...
    InputMsg =
        case hb_path:from_message(request, RawInputMsg, Opts) of
            ...
            Parts when is_list(Parts) ->
                case lists:member(nil, Parts) orelse lists:member(undefined, Parts) of
                    true -> RawInputMsg#{ <<"path">> => <<"compute">> };
                    false -> RawInputMsg
                end;
            _ -> RawInputMsg
        end,
    ...
    _ = file:write_file(
        "/tmp/dev_process_compute_path.log",
        io_lib:format("~p run_as_start slot=~p~n", [erlang:monotonic_time(millisecond), NextSlot]),
        [append]
    ),
    ...
    case Res of
        {ok, NewProcStateMsg} ->
            _ = file:write_file(
                "/tmp/dev_process_compute_path.log",
                io_lib:format("~p run_as_ok slot=~p~n", [erlang:monotonic_time(millisecond), NextSlot]),
                [append]
            ),
        ...
        {error, Error} ->
            _ = file:write_file(
                "/tmp/dev_process_compute_path.log",
                io_lib:format("~p run_as_error slot=~p~n", [erlang:monotonic_time(millisecond), NextSlot]),
                [append]
            ),
            {error, Error}
    end.
...
ensure_loaded(Base, Req, Opts) ->
    ...
    _ = file:write_file(
        "/tmp/dev_process_ensure_loaded.log",
        io_lib:format("~p ensure_loaded_start target=~p~n", [erlang:monotonic_time(millisecond), TargetSlot]),
        [append]
    ),
    ...
    case hb_ao:get(<<"initialized">>, Base, Opts) of
        ...
        _ ->
            ...
            _ = file:write_file(
                "/tmp/dev_process_ensure_loaded.log",
                io_lib:format("~p ensure_loaded_latest_start~n", [erlang:monotonic_time(millisecond)]),
                [append]
            ),
            LoadRes =
            ...
            _ = file:write_file(
                "/tmp/dev_process_ensure_loaded.log",
                io_lib:format("~p ensure_loaded_latest_done~n", [erlang:monotonic_time(millisecond)]),
                [append]
            ),
            ?event(compute,
                {snapshot_load_res,
            ...
            case LoadRes of
                {ok, MaybeLoadedSlot, MaybeLoadedSnapshotMsg} ->
                    _ = file:write_file(
                        "/tmp/dev_process_ensure_loaded.log",
                        io_lib:format("~p ensure_loaded_snapshot_found~n", [erlang:monotonic_time(millisecond)]),
                        [append]
                    ),
                    ...
                    _ = file:write_file(
                        "/tmp/dev_process_ensure_loaded.log",
                        io_lib:format("~p ensure_loaded_snapshot_normalized~n", [erlang:monotonic_time(millisecond)]),
                        [append]
                    ),
                    ?event(snapshot,
                ...
                not_found ->
                    _ = file:write_file(
                        "/tmp/dev_process_ensure_loaded.log",
                        io_lib:format("~p ensure_loaded_no_checkpoint~n", [erlang:monotonic_time(millisecond)]),
                        [append]
                    ),
                    ...
                    _ = file:write_file(
                        "/tmp/dev_process_ensure_loaded.log",
                        io_lib:format("~p ensure_loaded_init_start~n", [erlang:monotonic_time(millisecond)]),
                        [append]
                    ),
                    InitRes = init(Base, Req, Opts),
                    _ = file:write_file(
                        "/tmp/dev_process_ensure_loaded.log",
                        io_lib:format("~p ensure_loaded_init_done~n", [erlang:monotonic_time(millisecond)]),
                        [append]
                    ),
                    InitRes
            end
