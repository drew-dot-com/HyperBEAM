...
find_next_assignment(Base, Req, _Schedule, LastSlot, Opts) ->
    ...
    case LocalCacheRes of
        ...
        not_found ->
            _ = file:write_file("/tmp/dev_scheduler_debug.log", io_lib:format("~p find_next_assignment local_miss proc=~p slot=~p~n", [erlang:monotonic_time(millisecond), ProcID, LastSlot + 1]), [append]),
            {ok, RecvdAssignments} =
                hb_ao:resolve(
                    Base,
                    #{
            ...
            _ = file:write_file("/tmp/dev_scheduler_debug.log", io_lib:format("~p find_next_assignment remote_res count=~p~n", [erlang:monotonic_time(millisecond), map_size(RecvdAssignments)]), [append]),
            % Convert the assignments to an ordered list of assignments.
...
location(Base, Req, Opts) ->
    catch file:write_file('/tmp/scheduler_location_debug.log',
        io_lib:format(
            "SCHED_LOC_ENTRY method=~p path=~p~n",
            [hb_ao:get(<<"method">>, Req, <<"GET">>, Opts), hb_ao:get(<<"path">>, Req, <<"">>, Opts)]
        ),
        [append]
    ),
...
post_location(Base, RawReq, RawOpts) ->
    ...
    Signers0 = hb_message:signers(OnlyCommitted, Opts),
    Self =
        hb_util:human_id(
    ...
    RemoteAddr = hb_ao:get(<<"remote-addr">>, RawReq, undefined, Opts),
    IsLocal = (RemoteAddr == <<"127.0.0.1">>) orelse (RemoteAddr == <<"::1">>),
    Signers = case {Signers0, IsLocal} of
        {[], true} -> [Self];
        _ -> Signers0
    end,
    ExistingNonceRes = (catch hb_gateway_client:scheduler_location(Self, Opts)),
    ExistingNonce =
        case ExistingNonceRes of
    ...
    NewNonce = hb_ao:get(<<"nonce">>, OnlyCommitted, ExistingNonce + 1, Opts),
        ...
        io_lib:format(
            "SCHED_LOC_NONCE self=~p signers=~p existing=~p new=~p~n",
            [Self, Signers, ExistingNonce, NewNonce]
        ),
        [append]
    ),
    case {NewNonce > ExistingNonce, lists:member(Self, Signers)} of
        ...
        {true, true} ->
            ...
            dev_scheduler_cache:write_location(Signed, Opts),
                ...
                io_lib:format(
                    "SCHED_LOC_WRITE ok self=~p signers=~p url=~p nonce=~p ttl=~p~n",
                    [Self, Signers, hb_ao:get(<<"url">>, Signed, undefined, Opts), NewNonce, TimeToLive]
                ),
                [append]
            ),
            ?event(scheduler_location,
...
post_local_schedule(ProcID, PID, Req, Opts) ->
    ...
    % DrewFi: if a process's on-disk schedule is extended out-of-band (e.g. via
    % remote schedule caching), the per-process scheduling server can become
    % stale and start assigning already-occupied slots. Before we schedule, make
    % sure the server's `current` matches the cache high-water mark.
    {ok, PID2} = ensure_scheduler_server_uptodate(ProcID, PID, Opts),
        ...
        {true, <<"Process">>} ->
            ...
            ?event(
                {registering_new_process,
                    {proc_id, ProcID},
                    {pid, PID2},
                    {is_alive, is_process_alive(PID2)}
                }
            ),
            {ok, dev_scheduler_server:schedule(PID2, Req)};
        {true, _} ->
            ?event(
                {scheduling_message,
                    {proc_id, ProcID},
...
ensure_scheduler_server_uptodate(ProcID, PID, Opts) ->
    try
        Latest =
            case dev_scheduler_cache:latest(ProcID, Opts) of
                not_found -> -1;
                {Slot, _Chain} when is_integer(Slot) -> Slot
            end,
        Info = dev_scheduler_server:info(PID),
        Current =
            case Info of
                #{ current := C } when is_integer(C) -> C;
                _ -> -1
            end,
        case Latest > Current of
            true ->
                ?event(
                    scheduling,
                    {scheduler_server_stale_restart,
                        {proc_id, ProcID},
                        {current, Current},
                        {latest, Latest}
                    }
                ),
                catch dev_scheduler_server:stop(PID),
                {ok, dev_scheduler_registry:find(ProcID, true, Opts)};
            false ->
                {ok, PID}
        end
    catch
...
maybe_decode_ans104(Body, Opts) when is_binary(Body) ->
    % Allow clients to POST raw `application/ans104` payloads to `/schedule`
    % (common for AO), by decoding them into a structured message map.
    try dev_codec_ans104:deserialize(Body, #{}, Opts) of
        {ok, Msg} when is_map(Msg) -> Msg;
        _ -> Body
    catch
        _:_ -> Body
    end;
maybe_decode_ans104(Body, _Opts) ->
    Body.
...
drewfi_override_markers() ->
    'DREWFI_SCHEDULER_FALLBACK'.
