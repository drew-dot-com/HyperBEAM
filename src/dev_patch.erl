...
move(Mode, Base, Req, Opts) ->
    try
        _ = file:write_file(
            "/tmp/dev_patch.log",
            io_lib:format(
                "~p move_start mode=~p~n",
                [erlang:monotonic_time(millisecond), Mode]
            ),
            [append]
        ),
        maybe
        ...
        Class:Reason:Stack ->
            _ = file:write_file(
                "/tmp/dev_patch_error.log",
                io_lib:format(
                    "~p patch_error mode=~p class=~p reason=~p stack=~p~n",
                    [erlang:monotonic_time(millisecond), Mode, Class, Reason, Stack]
                ),
                [append]
            ),
