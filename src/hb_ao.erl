...
resolve_stage(5, Base, Req, ExecName, Opts) ->
	...
	{ResolvedFunc, NewOpts} =
		try
            ...
            case hb_opts:get(drewfi_debug_logs, false, Opts) of
                true ->
                    case hb_maps:get(<<"path">>, Req, not_found, Opts) of
                        <<"init">> ->
                            _ = file:write_file(
                                "/tmp/hb_ao_resolve.log",
                                io_lib:format(
                                    "~p stage5 path=~p exec=~p key=~p device=~p~n",
                                    [
            ...
            case hb_maps:get(<<"path">>, Req, not_found, Opts) of
                <<"init">> ->
                    _ = file:write_file(
                        "/tmp/hb_ao_resolve.log",
                        io_lib:format(
                            "~p stage6 path=~p exec=~p func=~p~n",
                            [erlang:monotonic_time(millisecond), <<"init">>, ExecName, Func]
                        ),
                        [append]
                    );
                <<"compute">> ->
                    _ = file:write_file(
                        ...
                        io_lib:format(
                            "~p stage6 path=~p exec=~p func=~p~n",
                            [erlang:monotonic_time(millisecond), <<"compute">>, ExecName, Func]
                        ),
                        [append]
                    );
                _ -> ok
...
get_first([{Base, Path}|Msgs], Default, Opts) ->
    case get(Path, Base, Opts) of
        ...
        nil -> get_first(Msgs, Default, Opts);
        undefined -> get_first(Msgs, Default, Opts);
        Value -> Value
    end.
