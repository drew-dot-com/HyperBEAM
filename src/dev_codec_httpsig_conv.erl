...
commitments_from_signature(Map, HPs, RawSig, RawSigInput) ->
    ...
    Commitments = maps:from_list(lists:map(
        fun ({SigName, Signature}) ->
            ...
            AddrOrB64 =
                case KeyIdBin of
                    <<"arweave:", Addr/binary>> -> {addr, Addr};
                    _ ->
                        case binary:split(KeyIdBin, <<":">>, [global]) of
                            [_, B64] -> {b64, B64};
                            _ -> {b64, KeyIdBin}
                        end
                end,
            Address = case AddrOrB64 of
                {addr, A} -> A;
                {b64, BVal} ->
                    try hb_util:human_id(ar_wallet:to_address(hb_util:decode(BVal))) catch _:__ -> BVal end
            end,
