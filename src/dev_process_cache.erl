...
read(ProcID, SlotRef, Opts) ->
    ...
    case hb_cache:read(Path, Opts) of
        {ok, Result} ->
            case cache_result_matches_proc(ProcID, Result, Opts) of
                true ->
                    {ok, Result};
                false ->
                    ?event({computed_cache_proc_mismatch, {requested, ProcID}, {path, Path}}),
                    not_found
            end;
        Other ->
            Other
...
cache_result_matches_proc(ProcID, Result, Opts) ->
    case hb_maps:get(<<"process+link">>, Result, not_found, Opts) of
        not_found ->
            true;
        Link ->
            Want = hb_util:human_id(hb_util:bin(ProcID)),
            Have = hb_util:human_id(hb_util:bin(Link)),
            Want == Have
    end.
