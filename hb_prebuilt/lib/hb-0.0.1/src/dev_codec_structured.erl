...
from(Msg, Req, Opts) when is_map(Msg) ->
    ...
    %% Compatibility shim: derive `committed` from `commitments` if enabled.
    Compat = hb_opts:get(structured_commit_compat, false, Opts),
    NormKeysMap0 = hb_ao:normalize_keys(NormLinks, Opts),
    NormKeysMap = case Compat of
        true ->
            case {maps:is_key(<<"committed">>, NormKeysMap0), maps:get(<<"commitments">>, NormKeysMap0, undefined)} of
                {false, Val} when is_map(Val) ->
                    ?event({compat_structured_commit, applied}),
                    NormKeysMap0#{ <<"committed">> => maps:keys(Val) };
                _ -> NormKeysMap0
            end;
        false -> NormKeysMap0
    end,
